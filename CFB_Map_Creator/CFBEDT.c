			...
			if( captured ){
				...
				switch (event.type) {
					...
					case SDL_KEYUP:
						...
						else if( selected_N > 0 && (event.key.keysym.sym == SDLK_x || 
					...
					case SDL_MOUSEBUTTONUP:{
						...
						if( ACTION >= TRANSLATING ){
							...
							if( ACTION == NONE &&
								...
								//printf("Selecting. sqrad: %g. ", sqrad);
								for(int i = 0; i < NM.N; i++){
									if( NM.status[i] & DELETED ) continue;
				...
				if( selection_altered && selected_N > 1 ){
					...
					int len = strlen(buf);
					for (int i = 0; i < selected_N; ++i ){
						if( i < selected_N-1 ) sprintf( buf+len, "%d, ", selected[i] );
						else sprintf( buf+len, "%d.", selected[i] );
						len = strlen(buf);
					}
					textbox_set_string( textbox, buf );
					DOCKED = TB;
					tb_state = 0;
				}
			}

...
int main( int argc, char *argv[] ){
	...
	while ( loop ) { //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> L O O P <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		switch( LOC ){
			...
			case 0: 
				CFB_Mensagem( R, &M, "Bom dia. Você está pronto? Não? Problema seu!" );
				//CFB_Mensagem( R, &M, "segunda mensagem" );
				break;
				
			case 1: {
				favorito = CFB_Escolhas( R, &M, "Decida-se!", 3, "Maçã", "Banana", "Sorvete" );
				if( favorito == 2 ){
					M.grafo.icon_IDs[ 1 ] = rand() % 36;
				}
				} break;
				
			case 2:{
				char str [32];
				sprintf( str, "4d6 + 1d1 + 1d13 > %d", 5 * favorito );
				DEVal resultado = CFB_Rolar_Dados( R, &M, "dados", str, NULL );
				if( resultado.total ){
					CFB_Mensagem( R, &M, "sucesso!" );
				}
				}break;
				
			case 3: 
				CFB_Imagem( R, &M, "Assets/GESBP pointer blue ver.png" );
				break;

...
void gp_draw_fastcircle(SDL_Renderer *R, float x, float y, float radius){
	if( radius < gp_circle_threshold8 ){
		gp_draw_8circle( R, x, y, radius );
	}
	else if( radius < gp_circle_threshold16 ){
		gp_draw_16circle( R, x, y, radius );
	}
	else if( radius < gp_circle_threshold24 ){
...
void gp_drawthick_fastcircle(SDL_Renderer *R, float x, float y, int C_radius, float L_radius ){
	if( C_radius < gp_circle_threshold8 ){
		gp_drawthick_8circle( R, x, y, C_radius, L_radius );
	}
	else if( C_radius < gp_circle_threshold16 ){
		gp_drawthick_16circle( R, x, y, C_radius, L_radius );
	}
	else if( C_radius < gp_circle_threshold24 ){
...
void gp_draw_Ncircle(SDL_Renderer *R, float x, float y, float radius, int N ){
	float px = x + radius;
	float py = y;
	float a = TWO_PI / N;
	for (int i = 1; i < N; ++i ){
		float nx = x + radius * cos( i * a );
		float ny = y + radius * sin( i * a );
		SDL_RenderDrawLineF( R, px, py, nx, ny );
		px = nx;
		py = ny;
	}
	SDL_RenderDrawLineF( R, px, py, x + radius, y );
}
void gp_drawthick_Ncircle(SDL_Renderer *R, float x, float y, int C_radius, float L_radius, int N ){
	SDL_Color C = SDL_GetRender_SDL_Color( R );
	SDL_Vertex *verts = malloc( 2 * N * sizeof(SDL_Vertex) );
	float a = TWO_PI / N;
	for (int i = 0; i < N; ++i ){
		verts[i  ] = (SDL_Vertex){ { cos( i * a ),        sin( i * a )        }, C, {0,0} };
		verts[i+N] = (SDL_Vertex){ { verts[i].position.x, verts[i].position.y }, C, {0,0} };
	}
	float rad = C_radius + L_radius;
	for (int i = 0; i < N; ++i ){
		verts[i].position.x = x + (rad * verts[i].position.x);
		verts[i].position.y = y + (rad * verts[i].position.y);
	}
	rad = C_radius - L_radius;
	for (int i = 0; i < N; ++i ){
		verts[i+N].position.x = x + (rad * verts[i+N].position.x);
		verts[i+N].position.y = y + (rad * verts[i+N].position.y);
	}

	int *indices = malloc( 6 * N * sizeof(int) );
	int o = 0;
	for (int i = 0; i < N-1; ++i ){
		indices[o++] = i;
		indices[o++] = i+1;
...
void gp_fill_Ncircle(SDL_Renderer *R, float x, float y, float radius, int N ){
	SDL_Color C = SDL_GetRender_SDL_Color( R );
	SDL_Vertex *verts = malloc( N * sizeof(SDL_Vertex) );
	float a = TWO_PI / N;
	for (int i = 0; i < N; ++i ){
		verts[i] = (SDL_Vertex){ { x + radius * cos( i * a ), y + radius * sin( i * a ) }, C, {0,0} };
	}
	int *indices = malloc( 3*(N-2) * sizeof(int) );
	int o = 0;
	for (int i = 1; i < N-1; ++i ){
		indices[o++] = 0;
		indices[o++] = i;
	...
	SDL_RenderGeometry( R, NULL, verts, N, indices, 3*(N-2) );
	free( verts );
	free( indices );
}
...
void gp_draw_pie(SDL_Renderer *R, float x, float y, float radius, float start, float end ){
	...
	float total = fabsf(end-start);
	if( total < 0.001 ){
		SDL_RenderDrawLineF( R, x, y, x + radius*cos(start), y + radius*sin(start) );
		return;
	} else if( total > TWO_PI ){
		gp_draw_fastcircle( R, x, y, radius );
		return;
	}
	int res = ceil( total * 5.7295779512896 );// / 0.1745329252 );
	float dt = total / res;

	//printf("<%g, %d, %g>\n", fabsf(end-start), res, dt );

	float tx = x + radius*cos(start);
	float ty = y + radius*sin(start);
	SDL_RenderDrawLineF( R, x, y, tx, ty );
	for (int i = 0; i <= res; ++i ){
		float nx = x + radius*cos(start + i * dt);
		float ny = y + radius*sin(start + i * dt);
		SDL_RenderDrawLineF( R, tx, ty, nx, ny );
		tx = nx;
		ty = ny;
	}
	SDL_RenderDrawLineF( R, tx, ty, x, y );
}

void gp_fill_pie(SDL_Renderer *R, float x, float y, float radius, float start, float end ){
	...
	if( total < 0.001 ){
		...
		gp_fill_fastcircle( R, x, y, radius );
		return;
	}
	int res = ceil( total * 7.63944 );
	float dt = total / res;
	...
	for (int i = 0; i < res; ++i ){
		verts[i] = (SDL_Vertex){ { x + radius*cos(start + (i * dt)), y + radius*sin(start + (i * dt)) }, C, {0,0} };
	}
	verts[res] = (SDL_Vertex){ { x, y }, C, {0,0} };
	...
	SDL_RenderGeometry(R, NULL, verts, res+1, indices, ind_N);
}
...
void gp_draw_roundedRect( SDL_Renderer *R, SDL_Rect *rect, float radius ){
	...
	SDL_RenderDrawLineF( R, xpr, rect->y, xwmr, rect->y );
	SDL_RenderDrawLineF( R, rect->x + rect->w, ypr, rect->x + rect->w, yhmr );
	SDL_RenderDrawLineF( R, xpr, rect->y + rect->h, xwmr, rect->y + rect->h );
	SDL_RenderDrawLineF( R, rect->x, ypr, rect->x, yhmr );

	double theta = HALF_PI / gp_rounded_res;

	float px1 = rect->x + rect->w;
	float px2 = rect->x + rect->w;
	float px3 = rect->x;
	float px4 = rect->x;
	float py1 = ypr;
	float py2 = yhmr;
	...
	for(int i = 0; i <= gp_rounded_res; i++){
		...
		float y3 = yhmr + sine;
		float y4 = ypr  - sine;
		SDL_RenderDrawLineF( R, px1, py1, x1, y1 );
		SDL_RenderDrawLineF( R, px2, py2, x2, y2 );
		SDL_RenderDrawLineF( R, px3, py3, x3, y3 );
		SDL_RenderDrawLineF( R, px4, py4, x4, y4 );
		px1 = x1; 
		px2 = x2; 
		px3 = x3; 
		px4 = x4; 
...
void gp_fill_roundedRect( SDL_Renderer *R, SDL_Rect *rect, float radius ){
	...
	SDL_Vertex verts[ gp_rounded_len ];
	double theta = HALF_PI / gp_rounded_res;
	...
	for(int i = 0; i <= gp_rounded_res; i++){
		...
		verts[  gp_rounded_res   - i] = (SDL_Vertex){ { xwmr + cosi, ypr  - sine }, C, {0,0} };
		verts[  gp_rounded_res+1 + i] = (SDL_Vertex){ { xwmr + cosi, yhmr + sine }, C, {0,0} };
		verts[3*gp_rounded_res+2 - i] = (SDL_Vertex){ { xpr  - cosi, yhmr + sine }, C, {0,0} };
		verts[3*gp_rounded_res+3 + i] = (SDL_Vertex){ { xpr  - cosi, ypr  - sine }, C, {0,0} };
	}

	int indices [ gp_rounded_tri ];
	for(int i = 0; i < gp_rounded_len-2; i++){
		indices[ 3*i   ] = 0;
		indices[ 3*i+1 ] = i+1;
		indices[ 3*i+2 ] = i+2;
	}
	//int out = 
	SDL_RenderGeometry(R, NULL, verts, gp_rounded_len, indices, gp_rounded_tri);
...
void gp_draw_poly( SDL_Renderer *R, vec2d *verts, int verts_count, bool close ){
	int n = verts_count - 1;
	for (int i = 0; i < n; ++i) SDL_RenderDrawLineF( R, verts[i].x, verts[i].y, verts[i+1].x, verts[i+1].y );
	if( close ) SDL_RenderDrawLineF( R, verts[n].x, verts[n].y, verts[0].x, verts[0].y );
}
